...
app.config.update(
    SESSION_COOKIE_SECURE=True,        # Cookies are sent only over HTTPS
    SESSION_COOKIE_HTTPONLY=True,      # Prevent client-side access to cookies
    SESSION_COOKIE_SAMESITE='Strict',  # Cookies restricted to same-site requests
)
...
def login():
    ...
    if request.method == "POST":
        ...
        cursor.execute('SELECT * FROM userdetails WHERE email = ?', (mailid,))
...
def register():
    if request.method == "POST":
        ...
        con = db_connection()
        cursor = con.cursor()
        cursor.execute('INSERT INTO userdetails (email, password) VALUES (?, ?)', (email, hashed_password))
        con.commit()
        con.close()
